...
extern bool CmpVec(const MVector& vec, const std::initializer_list<double>& init_list);

bool CmpMat(const MMatrix& mat, std::initializer_list<std::initializer_list<double>> il){
    if (mat.Rows() != il.size()) {
        return false;
    }
...
TEST(TestConstructorsMatrix, WithParams) {
    ...
    EXPECT_TRUE(CmpMat(mat, {{12, 12, 12, 12}, {12,12,12, 12}, {12, 12, 12, 12}}));
}
